...
| [vaultfire-trust](./vaultfire-trust/) | Vaultfire Protocol KYA (Know Your Agent) trust verification for AI coding agents | **Hooks:** SessionStart - Trust verification on startup, PreToolUse - Private key exposure monitoring<br>**Command:** `/vaultfire-trust` - Display trust status<br>**Agent:** `trust-verifier` - Trust data interpretation<br>**Skill:** `vaultfire-kya` - KYA trust framework guidance |
